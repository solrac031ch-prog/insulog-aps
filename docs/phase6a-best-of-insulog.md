...
Incorporar patrones útiles observados en sistemas digitales de titulación de insulina sin modificar el motor clínico `Clinical r2`, sin convertir Insulog en una plataforma de telemedicina y sin debilitar su política de privacidad.
...
La revisión no cambia la dosis, el estado clínico ni la nota generada. Su función es documentar la revisión humana antes de agregar un caso al historial temporal de la sesión.

### 3. Historial temporal de la sesión

Los casos pueden agregarse manualmente a una colección **efímera en memoria** durante la sesión actual, con un máximo de 100 registros. Cada registro conserva temporalmente alias/código local, fecha, versión de Insulog, revisión profesional, dosis previa/recomendada, HGT relevantes, explicación y nota clínica.

El historial:

- existe sólo durante la sesión actual;
- se borra al recargar o cerrar la aplicación;
- no utiliza mecanismos de persistencia del navegador;
...
Esto mantiene el invariant histórico de Insulog: **el estado clínico identificable permanece efímero en memoria**.
...
Una eventual persistencia longitudinal real deberá tratarse como una fase de privacidad y gobierno de datos separada, no como una modificación silenciosa del frontend.
...
3. el historial conserva la revisión durante la sesión y desaparece al recargar;
