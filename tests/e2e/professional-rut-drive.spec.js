...
test("la sesión profesional expone estado operativo claro y responsive", async ({ page }) => {
  ...
  const identity = page.locator("#professional-identity-bar");
  await expect(identity).toBeVisible();
  await expect(identity.locator(".professional-card-rut")).toContainText("678-5");
  await expect(identity.locator(".professional-card-badge")).toContainText("Activo");
  await expect(identity.locator("#professional-operational-status")).toHaveCount(0);

  const visual = await identity.locator(".professional-card").evaluate((node) => {
  ...
  expect(visual.display).toBe("flex");
