...
text = replace_once(text, '''  function setReviewStatus(message, ok = true) {
...
text = replace_once(text, '''      } else {
    ...
    if (decision === "reevaluar") setReviewStatus("Recomendación marcada para reevaluación clínica; no se emitirá documento con nueva pauta.", false);''', '''      } else {
    ...
    if (decision === "reevaluar") setReviewStatus("Recomendación marcada para reevaluación clínica; no se emitirá documento con nueva pauta.", false);''', "render-urgency-decision")

text = replace_once(text, '    if (helper) helper.textContent = "Clinical r2 mantiene su recomendación original. El profesional puede aceptarla, modificar la pauta dejando un motivo, o indicar reevaluación. La decisión profesional queda separada del cálculo del algoritmo.";', '    if (helper) helper.textContent = "Clinical r2 mantiene su recomendación original y sus alertas de seguridad. El profesional puede aceptarla, modificar la pauta dejando un motivo, o indicar reevaluación. Una alerta del algoritmo nunca sustituye el criterio clínico documentado del profesional.";', "helper-copy")

text = replace_once(text, '''  function validateModifiedPlan(am, pm, reason) {
    ...
    if (!Number.isInteger(am) || !Number.isInteger(pm) || am < 0 || pm < 0 || am > 150 || pm > 150) throw new Error("Ingrese dosis AM y PM enteras entre 0 y 150 UI.");''', '''  function validateModifiedPlan(am, pm, reason) {
    if (!Number.isInteger(am) || !Number.isInteger(pm) || am < 0 || pm < 0 || am > 150 || pm > 150) throw new Error("Ingrese dosis AM y PM enteras entre 0 y 150 UI.");''', "remove-urgency-hard-block")

text = replace_once(text, '''  function openModifyPanel() {
    ...
    const recommendation = currentRecommendation();''', '''  function openModifyPanel() {
    syncBaseClinicalNote();
    if (!baseClinicalNote) return;
    if (caseType(baseClinicalNote) === "urgencia") {
    ...
    const recommendation = currentRecommendation();''', "open-modify-on-urgency")

text = replace_once(text, '''    } else if (data.professionalDecision === "modificada") {
    ...
    } else if (data.professionalDecision === "reevaluar") {''', '''    } else if (data.professionalDecision === "modificada") {
      lines.push(`Decisión final del profesional: Modificada (${doseText(data.professionalAm, data.professionalPm)}).`);
      lines.push(`Motivo de modificación: ${String(data.professionalReason || "").trim()}`);
    ...
    } else if (data.professionalDecision === "reevaluar") {''', "document-urgency-override")

text = replace_once(text, '''  function canGeneratePatientDocument() {
    syncBaseClinicalNote();
    ...
    if (decision !== "aceptada" && decision !== "modificada") {''', '''  function canGeneratePatientDocument() {
    syncBaseClinicalNote();
    const decision = state.get("professionalDecision");
    ...
    if (decision !== "aceptada" && decision !== "modificada") {''', "document-guardrail-not-hard-block")

text = text.replace('window.InsulogPhase6B = Object.freeze({ version: "2026.09.14-phase6b", qualitySnapshot, canGeneratePatientDocument });', 'window.InsulogPhase6B = Object.freeze({ version: "2026.09.15-phase6b-clinician-override", qualitySnapshot, canGeneratePatientDocument });', 1)
path.write_text(text, encoding="utf-8")

path = Path("phase6b-document-sync.js")
text = path.read_text(encoding="utf-8")
text = replace_once(text, '''  function currentWeightInput() {
  ...
  }''', "robust-weight-source")
text = replace_once(text, '''      && reason.length >= 5
      && weight !== null
      && weight > 0
      && !isUrgencyRoute();''', '''      && reason.length >= 5
      && weight !== null
      && weight > 0;''', "allow-urgency-overbasal-warning")
text = text.replace('// si existe justificación clínica documentada. La ruta de urgencia continúa bloqueada.', '// si existe justificación clínica documentada. Las alertas de urgencia se conservan, pero no anulan una decisión profesional explícita.', 1)
text = text.replace('version: "2026.09.14-phase6b-document-sync-professional-override"', 'version: "2026.09.15-phase6b-document-sync-clinician-override"', 1)
path.write_text(text, encoding="utf-8")

path = Path("docs/phase6b-professional-decision.md")
text = path.read_text(encoding="utf-8")
text = text.replace('`REEVALUAR` y las rutas de urgencia no permiten emitir una nueva pauta ambulatoria desde esta capa.', '`REEVALUAR` no permite emitir una nueva pauta ambulatoria. Si Clinical r2 activa una ruta de urgencia, `ACEPTAR` conserva esa conducta; `MODIFICAR PLAN` permite al profesional registrar y emitir una pauta alternativa siempre que deje una justificación clínica explícita. La alerta original permanece visible y documentada.')
text = text.replace('El umbral de 0,5 UI/kg/día sigue siendo un guardrail para la recomendación automática. Una pauta manual >0,5 UI/kg/día puede registrarse y emitirse cuando el profesional la modifica de forma explícita y deja una justificación clínica; Insulog mantiene una advertencia visible y documenta que se trata de una excepción por criterio profesional. Las rutas de urgencia siguen bloqueadas.', 'El umbral de 0,5 UI/kg/día y las rutas de urgencia siguen siendo guardrails de la recomendación automática. No son bloqueos absolutos de la decisión médica: una pauta manual puede apartarse de ellos cuando el profesional usa `MODIFICAR PLAN` y deja una justificación clínica explícita. Insulog mantiene las advertencias originales y documenta claramente la excepción por criterio profesional.')
path.write_text(text, encoding="utf-8")

Path("tests/e2e/phase6b-urgency-professional-override.spec.js").write_text(r'''const { test, expect } = require("@playwright/test");
...
print("Clinician override patch prepared")
