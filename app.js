  ...
  function definirEsquemaInicio() {
    ...
    if (caja) {
      const decisionMessage = professionalInitiation
        ? "<br><br><strong>Decisión clínica:</strong> Insulog no identificó un criterio automático de inicio; el profesional puede definir esquema y factor."
        : "";
      caja.innerHTML = `<strong>Esquema sugerido:</strong> ${notePresenter.escapeHTML(decision.schemeText)}<br><br><strong>Motivo:</strong> ${notePresenter.escapeHTML(decision.reason)}${decisionMessage}`;
      show(caja, true);
    }
