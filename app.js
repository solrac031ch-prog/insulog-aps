...
(() => {
  ...
  function definirEsquemaInicio() {
    ...
    if (caja) {
      caja.innerHTML = `<strong>Esquema sugerido:</strong> ${notePresenter.escapeHTML(decision.schemeText)}<br><strong>Factor sugerido:</strong> ${Number(decision.factor || 0.2).toFixed(1).replace(".", ",")} UI/kg<br><strong>Motivo:</strong> ${notePresenter.escapeHTML(decision.reason)}`;
      show(caja, true);
    }
  ...
  function mostrarResumenEsquemaInicio() {
    ...
    caja.innerHTML = `<strong>Esquema sugerido:</strong> ${notePresenter.escapeHTML(data.textoEsquemaInicioSugerido || data.textoEsquemaInicio)}<br><strong>Factor sugerido:</strong> ${Number(data.factorInicioSugerido || 0.2).toFixed(1).replace(".", ",")} UI/kg<br><strong>Motivo:</strong> ${notePresenter.escapeHTML(data.motivoEsquemaInicio)}`;
    show(caja, true);
  }
