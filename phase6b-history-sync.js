...
(() => {
  ...
  if (!runtime) throw new Error("InsulogRuntime debe cargarse antes de phase6b-history-sync.js");
  if (!phase6b) throw new Error("InsulogPhase6B debe cargarse antes de phase6b-history-sync.js");
  if (!notePresenter) throw new Error("InsulogNotePresenter debe cargarse antes de phase6b-history-sync.js");
  ...
  function totalDose(dose = {}) {
    return (safeNumber(dose.am) || 0) + (safeNumber(dose.pm) || 0);
  }
  ...
  function currentAlias() {
    return String(byId("best-history-alias")?.value || "").trim().slice(0, 60);
  }
  ...
  function saveHistory() {
    try {
      ...
      setStatus(`✓ Caso guardado en esta sesión: ${record.alias}`, true);
      return record;
    } catch (error) {
      setStatus(error?.message || "No se pudo guardar el caso.", false);
      return undefined;
    }
  }
  ...
  function registerActions() {
    actions.register("best-history-save", saveHistory, { replace: true });
  ...
  window.InsulogPhase6BHistory = Object.freeze({
    version: "2026.09.14-phase6b-history-sync",
    read: () => sessionHistory.slice(),
